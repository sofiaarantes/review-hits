                  ...
                  <div class="card-body">
                    ...
                    <form action="{{ route('musicas.update', $musica->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="titulo" name="titulo" placeholder="Digite o título da música" value="{{ old('titulo', $musica->titulo) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="album" class="form-label">Nome do álbum ao qual a música pertence</label>
                            <input type="text" class="form-control" id="album" name="album" placeholder="Digite o nome do álbum da música" value="{{ old('album', $musica->album) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="ano_lancamento" class="form-label">Ano que música foi lançada</label>
                            <input type="number" class="form-control" id="ano_lancamento" name="ano_lancamento" placeholder="Digite o ano de lançamento música" value="{{ old('ano_lancamento', $musica->ano_lancamento) }}" required>
                        </div>
                        <div class="form-group">
                          <label for="genero_id">Gênero da música</label>
                          <select class="form-control" id="genero_id" name="genero_id" required>
                            <option value="">Selecione</option>
                            @foreach($generos as $genero)
                              <option value="{{ $genero->id }}" 
                                {{ old('genero_id', $musica->genero_id) == $genero->id ? 'selected' : '' }}>
                                {{ $genero->nome_genero }}
                              </option>
                            @endforeach
                          </select>
                        ...
                        <div class="form-group">
                          <label for="artista_id">Artista autor da música</label>
                          <select class="form-control" id="artista_id" name="artista_id" required>
                            <option value="">Selecione</option>
                            @foreach($artistas as $artista)
                              <option value="{{ $artista->id }}" 
                                {{ old('artista_id', $musica->artista_id) == $artista->id ? 'selected' : '' }}>
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('musicas.index') }}" class="btn btn-secondary me-2">Cancelar</a>
